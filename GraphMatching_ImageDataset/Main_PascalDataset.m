...
asgGagm = cell(size(outliers));
   ...
   for i = 1:size(outliers, 2)
        if ~exist(gmat, 'file')
            [K, group1, group2, KP, KQ, gphs, X_GT,Graph1, Graph2] = conPascalGphs(fmat, outliers(i));
            GRAPH{i}.K = K;
            GRAPH{i}.KP = KP;
            GRAPH{i}.KQ = KQ;
            ...
            GRAPH{i}.gphs = gphs;
            GRAPH{i}.X_GT = X_GT;
            GRAPH{i}.Graph1=Graph1;
            GRAPH{i}.Graph2=Graph2;
        else
            K = GRAPH{i}.K;
            KP = GRAPH{i}.KP;
            KQ = GRAPH{i}.KQ;
            ...
            gphs = GRAPH{i}.gphs;
            X_GT = GRAPH{i}.X_GT;
            Graph1= GRAPH{i}.Graph1;
            Graph2= GRAPH{i}.Graph2;
        end
        asgT.X = X_GT;
        ...
        lambda=0.005;num=11;
        %KP=zeros(size(KP));
        [X,val]=KerGM_Exact(Graph1,Graph2,KP,KQ,lambda,num);
        asgKergm{i}.tm = cputime - t0;
        asgKergm{i}.alg = 'KerGM';
        asgKergm{i}.X = X;
        asgKergm{i}.acc = matchAsg(X, asgT);
        asgKergm{i}.obj = X(:)' * K * X(:);
        fprintf('\r\ncImg = %d, outliers = %d, KerGM finish!', cImg, outliers(i));

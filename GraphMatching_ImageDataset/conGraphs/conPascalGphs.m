function [K, group1, group2, KP, KQ, gphs, X_GT,Graph1,Graph2] = conPascalGphs(fmat, outliers)
load(fmat);
...
[Graph1.G,Graph1.H,Graph2.G,Graph2.H]=PrecomputingPascal(gphs);
Graph1.K=SelfAffinityMat(gphs{1,1},m1,p1,q1);
Graph2.K=SelfAffinityMat(gphs{1,2},m2,p2,q2);

end

function [G1,H1,G2,H2]=PrecomputingPascal(gphs)
...
function [K]=SelfAffinityMat(g,m,p,q)
% Input:
...
for i=1:m
    ...
    % The attributes of the ith edge (p(i),q(i))
    cor_p = g.Pts(p(i), 1:2);
    cor_q = g.Pts(q(i), 1:2);
    y_dist = abs(cor_p(1) - cor_q(1)) / S(1);
    x_dist = abs(cor_p(2) - cor_q(2)) / S(2);
    dsti = sqrt(y_dist * y_dist + x_dist * x_dist); 
    angi = atan(y_dist / (x_dist + eps));

    for j=i+1:m
        
        % The attributes of the jth edge (p(j),q(j))
        cor_p = g.Pts(p(j), 1:2);
        cor_q = g.Pts(q(j), 1:2);
        y_dist = abs(cor_p(1) - cor_q(1)) / S(1);
        x_dist = abs(cor_p(2) - cor_q(2)) / S(2);
        dstj = sqrt(y_dist * y_dist + x_dist * x_dist); 
        angj = atan(y_dist / (x_dist + eps));
        
        
        %% The similarity between edgei and edgej
        Dst = abs(dsti - dstj);
        Ang = abs(abs(angi) - abs(angj));
        K(i,j) = exp(-(Dst + Ang) / 2);
